on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/io.github.rustadministrator.rustadmin_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/io.github.rustadministrator.rustadmin_service.plist"
  set app_bundle to "/Applications/RustAdmin.app"
  ...
  set kill_others to "pids=$(pgrep -x 'RustAdmin' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "RustAdmin wants to update itself" with administrator privileges
end run
