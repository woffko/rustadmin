...
pub static ref T: std::collections::HashMap<&'static str, &'static str> =
    [
        ...
        ("enable-quic-tip", "Использовать QUIC поверх UDP для прямых подключений по IP и через VPN. Если UDP недоступен, выполняется откат на TCP. При ошибке сертификата или идентификации узла откат запрещён."),
